...
public class FollowThePath : MonoBehaviour {
    ...
    private void Update()
    {
        if (movingIsActive)
        {
            ...
            {
                MovementSpeed = Mathf.Clamp(MovementSpeed + speed * Time.deltaTime, 0, speed);
                currentPathPercent += MovementSpeed / 100 * Time.deltaTime;
                ...
                gradient.SetKeys(
                    new GradientColorKey[] {
                        new GradientColorKey(FillingColor, currentPathPercent), new GradientColorKey(Color.gray, 1.0f) },
                        Road.colorGradient.alphaKeys
                );
                gradient.mode = GradientMode.Fixed;
                Road.colorGradient = gradient;
                ...
                if (currentPathPercent > 1)                    //when the path is complete
                {
                    ...
                    {
                        WinWindow.SetActive(true); movingIsActive = false;
                    }
                }
            }
            else {
